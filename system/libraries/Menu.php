...
class CI_Menu
{
	...
	*/
    ...
    public function __construct()
    {
		...
		$config["nav_tag_open"]          = '<ul class="sidebar-menu" data-widget="tree"><li class="header">MENU SISTEMA FLOTA</li>';
   	...
   	private function render_item($items, &$html = '')
	{
		...
		foreach ($items as $item) {
			if ( isset($item[$this->menu_label], $item['KEY']) ) 
	        {
        		...
        		// die(var_dump($item[$this->menu_id]));
        		if ( $icon) {
    				$icon = "<i class='{$icon}'></i>";
