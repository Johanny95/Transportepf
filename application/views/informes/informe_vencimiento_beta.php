					...
					<div class="row ">
						...
						<div class="col-sm-3">
							<div class="form-group">
								...
								<select class="form-control selectpicker show-tick" data-placeholder="Seleccione Tipo de Documento"
								style="width: 100%;" id="tipoDocumento" name="tipoDocumento" data-live-search="true" data-actions-box="true" data-selected-text-format="count"> 
					...
					<div class="col-sm-3">
						<?php if ($user[0]['OFICOD']==='ALL'): ?>
							<div class="form-group">
								...
								<select id='oficina' name='oficina' class="form-control select2" style="width: 100%">
									<option value="" selected>Ver todos</option>
									<?php foreach ($getOficinas as $key): ?>
										<?php if ($key->NOMBRE_OFICINA == ( !isset($oficina) ? "" : $oficina )) : ?>
											<option  selected="TRUE" value="<?php echo $key->NOMBRE_OFICINA?>"><?php echo $key->NOMBRE_OFICINA?></option>
											<?php else: ?>
												<option value="<?php echo $key->NOMBRE_OFICINA?>"><?php echo ucwords(mb_strtolower($key->NOMBRE_OFICINA))?></option>
											<?php endif ?>
										<?php endforeach ?>
									</select>
								</div>
	...
	<div class="modal fade " id="modal_info_aprob" >
		...
		<script>
    		...
    		t = $('#tablaInformeVigencia').DataTable({
    			"ajax": {
    				"url": "<?php echo site_url() ;?>/tabla_resumen/get_informe_vigencia",
    				'type': 'POST',
    				"dataSrc":"",
    				"data": function ( d ) {
